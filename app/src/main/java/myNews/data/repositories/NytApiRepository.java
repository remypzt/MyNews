...
public class NytApiRepository {
	...
	public MutableLiveData<List<Articles>> getTopStories(String section) {
		...
		mNytApiInterfaceService
				...
				.enqueue(new Callback<ResponseOfTopStories>() {
					...
					@Override
					public void onFailure(Call<ResponseOfTopStories> call,
					                      Throwable t) {
						topStories.setValue(null);
					}
				});
	...
	public MutableLiveData<List<Articles>> getMostPopular() {
		...
		mNytApiInterfaceService
				.getResponseOfMostPopular()
				.enqueue(new Callback<ResponseOfMostPopular>() {
					@Override
					public void onResponse(Call<ResponseOfMostPopular> call,
					                       Response<ResponseOfMostPopular> response) {
						if (response.isSuccessful()) {
					...
					@Override
					public void onFailure(Call<ResponseOfMostPopular> call,
					                      Throwable t) {
						mostPopular.setValue(null);
					}
				});
	...
	public MutableLiveData<List<Articles>> getSearchArticles(String query,
	                                                         String filter,
	                                                         String beginDate,
	                                                         String endDate) {
		MutableLiveData<List<Articles>> searchArticles = new MutableLiveData<>();
		mNytApiInterfaceService
				.getResponseOfArticleSearch(query, filter, beginDate, endDate)
				.enqueue(new Callback<ResponseOfArticleSearch>() {
					@Override
					public void onResponse(Call<ResponseOfArticleSearch> call,
					                       Response<ResponseOfArticleSearch> response) {
						if (response.isSuccessful()) {
					...
					@Override
					public void onFailure(Call<ResponseOfArticleSearch> call,
					                      Throwable t) {
						searchArticles.setValue(null);
					}
				});
