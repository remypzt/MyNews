   ...
   /* public static List<Articles>  generateArticlesFromArticleSearch(ResponseOfArticleSearch responseOfArticleSearch)
    {
        ...
        {
            List<ResponseOfArticleSearch> resultsArticleSearch = responseOfArticleSearch.getResponseOfArticleSearch();

            for (int x = 0; x <= resultsArticleSearch.size() - 1; x++)
                articleSearchArticles.add(addArticleFromArticleSearch(resultsArticleSearch.get(x)));

        }
        return articleSearchArticles;
    }

    private static Articles addArticleFromArticleSearch(ResponseOfArticleSearch responseOfArticleSearch)
    {
